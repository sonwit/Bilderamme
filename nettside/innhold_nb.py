# Alt som er tekst på nettsiden, på norsk. Den engelske utgaven er innhold_en.py
...
SPRAAK_NAVN = "Norsk"
SPRAAK_LABEL = "Språk"
ROT = ""                          # norsk ligger i roten av siden, engelsk under en/
TITTEL = "Fugleramme"
...
# Veggsiden: faste ord. Stedet er bare et ord, ikke en adresse.
VEGG = {
    "kicker": "Fugler i dag",
    "sted": "Hagen",
    "hoert": "Hørt i dag",
...
DAG_KORT = "{u} {d}."             # Tor 24., i stripen med siste dager
DESIMAL = ","

# Dagfilene skrives på norsk av serveren, så her er det ingenting å oversette.
# Den engelske filen har tabellene som gjør jobben der.
...
ART = {
    ...
    "plass_stor": "Får den tykke grenen nederst.",
    "plass_midt": "Får en av grenene i midten.",
    "forelegg": "Forelegg",
...
KOMPONENTER = [
    ("Utedel", "firmware/outdoor_sensor/",
     "Tar opp ett minutt rett i PSRAM med radioen av, kobler til, synker klokken, laster opp og sover. Deep sleep er 14 µA.",
     ...
     "firmware/README.md"),
    ("Hjemmeserver", "tools/",
     "Tar imot lyd og bilder, kjører BirdNET, regner lytteplanen, komponerer siden, dithrer og pusher. Websidene er ren standardbibliotek.",
...
VALG = [
    ...
    ("Lytteplanen følger solen og batteriet",
     "En fast plan treffer morgenkoret i juni og bekmørke i desember.",
     "Fast morgenrunde fra klokken fire, som i firmwaren.",
    ...
    ("Vakten ser etter stillhet",
     ...
     "Vakten ser etter det som ikke skjer, og er ellers helt stille. Én melding når noe er galt."),
    ("07:07, ikke 07:00",
     "Vær-API-et er mest overbelastet på hel time.",
     "Cron på 07:00.",
     "To dagers 503-stopp i juli 2026.",
     "Sju over sju. Og været stopper aldri siden: tre forsøk, så tegnes den uten værreferanse."),
]
FORKASTET = [
    ...
    ("Modellen komponerer hele plansjen",
     "Den tegnet sin egen gren midt på siden uansett hva referansen sa. Koden tok over komposisjonen."),
...
BYGGET = {
    ...
    "stroem": "Strømmen ute",
    "stroem_avsnitt": ("Solcellen var netto positiv i august med 24 økter om dagen. Planen trappes ned av seg selv "
...
TIDSLINJE = [
    ...
    ("28. august", "Fuglesiden tar over for det frie AI-bildet."),
...
BUNN = {
    "laget_foer": "Laget av ",
    "bruker": "sonwit",
    "laget_etter": ". Alt fra firmware til denne siden ligger åpent på GitHub.",
    ...
    "sporing": "Siden bruker ingen sporing.",
    "hopp": "Til innholdet",
}
