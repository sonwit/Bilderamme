...
fugler på en fast gren i et fast oppsett, dithrer siden til panelets seks farger
...
(rammen) og august 2026 (utedelen og fuglesiden).
...
```
 ...
 [Hjemmeserver]   /opt/fugleramme, to venv-er, systemd + cron
     ...
     daily_panel.py:  compose_branch ──► render_daily_panel ──► render_panel_png
                      fuglene på grenen   siden som HTML         PNG → dither → frame.bin
...
| Server | Alt er Python, og sidene som serveres er ren standardbibliotek. Den daglige kjeden, lyd og arter, webappen, helsesiden og vakten. | [tools/README.md](tools/README.md), [deploy/README.md](deploy/README.md) |
...
uforutsigbart. Fuglesiden er det motsatte: samme oppsett hver dag, artene som
faktisk ble hørt, med norsk og latinsk navn, klokkeslett og sikkerhet. Det
frie bildet lever videre som reserve når fuglesiden feiler, og som det Siri og
...
sin egen midt på siden. Derfor tegner modellen én fugl om gangen, én gang per
...
Fuglene skaleres etter kroppslengde, med en komprimert skala så gråhegren ikke
...
og grønt. Alt annet må dithres, og dithret tekst er grøt. Hele fuglesiden er
...
**Lytteplanen følger solen og batteriet.** Soloppgangen i Oslo-området
...
**Standardbibliotek på serveren.** Helsesiden, fuglesiden, dagsoversikten og
vakten er ren `stdlib`, med grafer som håndtegnet SVG. Da kjører de i begge
...
merket ingen det, for det kom jo ingen nye filer å reagere på. Vakten ser
...
3. **Sett opp serveren.** `/opt/fugleramme`, to venv-er, `frame_server.env`
   fra eksempelet i `deploy/`, systemd-tjenestene og cron-linjen. Se
...
Koordinatene for hagen, stedsnavnet på siden og kontakten i User-Agent leses fra
...
| `nettside/` | Produktsiden på GitHub Pages, bygd fra repoets data. [README](nettside/README.md) |
| `docs/` | Guider og prosjektnotater, med historikk. [Innhold](docs/README.md) |
| `test/` | Testdata: en dags `birds.json` til å rendre siden med lokalt. |
